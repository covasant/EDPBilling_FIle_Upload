...
def resolve_selection(selectors: list[dict] | None) -> list[SpeedeFile]:
    """The catalogue entries a request asks for, in catalogue order. None or an
    empty list means all 12."""
    if not selectors:
    ...
    # call can never interleave two files into one shared category.
    return [entry for entry in CATALOGUE if entry in chosen]
...
def _upload_one(
    ...
    """
    file_path = _locate(entry, trade_date)
...
def _process_entry(
    ...
    record = repo.find(trade_date, entry.account, entry.report)
    if record is None:
        record = repo.insert(
    ...
    try:
        ...
        return _upload_one(
            repo, client, record, entry, category, settings_cache[category.upload_id], trade_date
        )
...
def process_upload(
    ...
    key = f"nsdl_speede|{trade_date}|{len(entries)}"
