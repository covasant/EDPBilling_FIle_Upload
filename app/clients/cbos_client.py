...
One file-upload batch (one segment + one trade date):
  ...
  Step 2 - getNewTradeProcess (PROCESSID=0)              -> PROCESSID + Table2 (UploadID candidates)
  Step 3 - CheckProcessIDExist                           -> confirms Step 2's PROCESSID registered
  Step 4 - GetNewTradeProcessPromodalUploadSettings      -> per-UploadID pattern/column rules
  Step 5 - SaveTradePromodalUploadChunkFile              -> one call per CHUNK_SIZE_KB chunk
  Step 6 - getdropdown(EXISTINGPROCESSID)                -> optional confirmation lookup
...
def _summarise(response) -> str:
    ...
    if len(text) <= _LOG_BODY_LIMIT:
        return text
    truncated = len(text) - _LOG_BODY_LIMIT
    return f"{text[:_LOG_BODY_LIMIT]}... [+{truncated} chars, LOG_LEVEL=DEBUG for full body]"
...
def _parse_upload_rule(upload_id: str, setting: dict, fallback_name: str = "") -> UploadRule | None:
    ...
    extension = (
        str(setting.get("FILEEXTENSION") or setting.get("FileExtension") or "")
        .strip()
        .lstrip(".")
        .upper()
    )
    ...
    if raw_columns not in (None, "", "-"):
        try:
            ...
            logger.warning(
                "Non-numeric column count %r for UPLOADID=%s, ignoring", raw_columns, upload_id
            )
...
class BaseCBOSClient(ABC):
    ...
    def _create_file_entry(
        self, upload_id: str, guid: str, file_name: str, process_id: str, trade_date: str
    ) -> dict:
        """Step 7 raw call."""
    ...
    def reserve_process(
        self, segment: str, trade_date: str, process_id: str = "0"
    ) -> ProcessReservation:
    ...
    def may_begin_upload(self, segment: str, trade_date: str) -> bool:
        ...
        msg = self._gtg_msg(
            self._call(
                1,
                "file_process_status(BeginFileUpload)",
        ...
        logger.info(
            "Step 1 BeginFileUpload segment=%s MSG=%s -> %s",
            segment,
            msg,
    ...
    def upload_settings(self, upload_id: str, fallback_name: str = "") -> UploadRule | None:
        ...
        raw = self._call(
            4,
            "GetNewTradeProcessPromodalUploadSettings",
            ...
            for current_chunk in range(total_chunks):
                ...
                for attempt in range(1, retries + 1):
                    try:
                        ...
                        # Loop variables bound as lambda DEFAULTS (B023): the
                        # lambda is invoked synchronously today, but late
                        # binding would silently resend the LAST chunk for
                        # every position if _call ever deferred it.
                        self._call(
                            5,
    ...
    def find_existing_process_id(self, segment: str, trade_date: str) -> str | None:
        ...
        try:
            ...
            logger.warning(
                "find_existing_process_id: getdropdown(EXISTINGPROCESSID) failed (non-fatal): %s",
                exc,
            )
            return None
    ...
    def register_file(
        ...
        self._call(
            7,
            "SaveNewTradeProcessPromodalUploadFile",
...
class CBOSClient(BaseCBOSClient):
    ...
    def _post_multipart(self, url: str, data: dict, files: dict) -> dict:
        logger.debug(
            "Request -> %s: data=%s file=%s", url, _redact(data), files.get("file", (None,))[0]
        )
        try:
            response = requests.post(
                url, data=data, files=files, timeout=settings.cbos_upload_timeout_seconds
            )
        except requests.RequestException as exc:
            logger.error("Request -> %s failed: %s", url, exc)
            raise CBOSUploadError(f"Request to {url} failed: {exc}") from exc
    ...
    def _begin_file_upload(self, segment: str, trade_date: str) -> dict:
        ...
        payload = {
            "Segment": segment,
            "TradeDate": _to_cbos_date(trade_date),
            "ProcessName": PROCESS_NAME_BEGIN_FILE_UPLOAD,
    ...
    def _check_process_id_exist(self, segment: str, trade_date: str) -> dict:
        ...
        payload = {
            "Segment": segment,
            "TradeDate": _to_cbos_date(trade_date),
            "ProcessName": PROCESS_NAME_CHECK_PROCESS_ID,
            "UserID": settings.cbos_login_id,
        }
        return self._post(self._gtg_url(FILE_PROCESS_STATUS_PATH), payload)
    ...
    def _file_upload_status(self, segment: str, trade_date: str) -> dict:
        ...
        payload = {
            "Segment": segment,
            "TradeDate": _to_cbos_date(trade_date),
            "ProcessName": PROCESS_NAME_FILE_UPLOAD_STATUS,
            "UserID": settings.cbos_login_id,
        }
        return self._post(self._gtg_url(FILE_PROCESS_STATUS_PATH), payload)
...
class MockCBOSClient(BaseCBOSClient):
    def __init__(self) -> None:
        ...
        self.marked_optional: list[
            tuple
        ] = []  # (process_id, step_no) per Step-8 call, for assertions
        ...
        self._filled_upload_ids: dict[
            str, set
        ] = {}  # process_id -> UPLOADIDs already registered (Step 7)
    ...
    def _get_new_trade_process(self, segment: str, trade_date: str, process_id: str = "0") -> dict:
        ...
        if process_id and str(process_id) != "0":
            logger.debug(
                "[MOCK] Reusing existing PROCESSID=%s (GROUPNAME=%s, TRADEDATE=%s)",
                process_id,
                segment,
                trade_date,
            )
            ...
            logger.debug(
                "[MOCK] Process ID created: PROCESSID=%s (GROUPNAME=%s, TRADEDATE=%s)",
                process_id,
                segment,
                trade_date,
            )
    ...
    def _check_process_id_exist(self, segment: str, trade_date: str) -> dict:
        ...
        logger.debug(
            "[MOCK] Check process id exist: segment=%s trade_date=%s -> PROCESSID=%s",
            segment,
            trade_date,
            pid,
        )
    ...
    def _upload_chunk(
        ...
        logger.debug(
            "[MOCK] Chunk uploaded: %s chunk %d/%d (upload_id=%s, guid=%s)",
            file_name,
            current_chunk,
            total_chunks,
            upload_id,
    ...
    def _get_existing_process_id(self, segment: str, trade_date: str) -> dict:
        ...
        logger.debug(
            "[MOCK] Existing process id lookup: segment=%s trade_date=%s -> %s",
            segment,
            trade_date,
            pid,
        )
    ...
    def _create_file_entry(
        ...
        logger.debug(
            "[MOCK] File entry created: %s (upload_id=%s, guid=%s)", file_name, upload_id, guid
        )
    ...
    def _begin_file_upload(self, segment: str, trade_date: str) -> dict:
        ...
        logger.debug(
            "[MOCK] BeginFileUpload segment=%s trade_date=%s -> %s", segment, trade_date, msg
        )
    ...
    def _file_upload_status(self, segment: str, trade_date: str) -> dict:
        ...
        if state["attempts"] <= settings.cbos_mock_pending_polls:
            logger.debug(
                "[MOCK] FILEUPLOAD FALSE/pending (attempt %d/%d, segment=%s)",
                state["attempts"],
                settings.cbos_mock_pending_polls,
                segment,
            )
    ...
    def get_expected_filename(self, segment: str, upload_id: str) -> dict:
        ...
        return {
            ...
            "Data": [
                {
                    "UploadID": upload_id,
                    "ExpectedFileNamePattern1": data.expected_pattern(upload_id),
                }
            ],
        }
...
def get_cbos_client() -> BaseCBOSClient:
    ...
    if _client is None:
        ...
        if mode == "REAL":
            ...
            raise CBOSUploadError(
                f"Invalid CBOS_MODE '{settings.cbos_mode}' - must be MOCK or REAL"
            )
